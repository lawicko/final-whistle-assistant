...
const pluginNodeClass = "MidfieldContributionPlugin"
...
const callback = (mutationList, observer) => {
    ...
    if (tableNode != undefined && tableNode.rows.length > 1) {
        ...
        console.log("Found the following table: ",tableNode)
        console.log("tableNode.rows.length: ", tableNode.rows.length)
        ...
        console.log("starting the table observation...")
        o.observe(tableNode, { childList: true, subtree: true, characterData: true })
    } else {
        console.log("Could not find the table, or the table is empty, observing...")
    }
};
